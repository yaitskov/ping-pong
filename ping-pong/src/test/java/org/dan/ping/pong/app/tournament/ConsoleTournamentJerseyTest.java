...
import static org.dan.ping.pong.app.playoff.PlayOffGuests.LosersUpToSemifinals;
...
import static org.dan.ping.pong.app.tournament.TournamentRulesConst.RULES_JP_S1A2G11_NP_3P;
...
public class ConsoleTournamentJerseyTest extends AbstractSpringJerseyTest {
    ...
    public static ImperativeSimulator playMasterG3Q1(ImperativeSimulator c) {
        return c
                .scoreSet(p1, 11, p3, 4)
                .scoreSet(p2, 11, p3, 5)
                .scoreSet(p1, 11, p2, 3)
    ...
    @Test
    public void mergedLayeredConsoleForPlayOffWithoutSemiFinalLosers() {
        final TournamentScenario scenario = begin().name("mrgdLrdForPlayOff-p3p4")
                .rules(RULES_JP_S1A2G11_NP_3P.withPlayOff(
                        RULES_JP_S1A2G11_NP_3P.getPlayOff()
                                .map(pr -> pr.withConsoleParticipants(
                                        Optional.of(LosersUpToSemifinals)))))
                .category(c1, p1, p2, p3, p4, p5, p6, p7, p8);
        isf.create(scenario)
                .run(master -> {
                    master.beginTournament()
    ...
    public void mergedLayeredConsoleForPlayOffW3() {
        ...
        isf.create(scenario)
                .run(master -> {
                    ...
                    console.checkTournament(Open, restState(Play).bid(p4, Wait).bid(p3, Wait))
                            .scoreSet(p5, 11, p8, 3)
                            .scoreSet(p6, 11, p7, 1)
                            .reloadMatchMap()
                            .scoreSet(p5, 11, p6, 1)
                            ...
                            .checkResult(p3, p4, p5, p6, p8, p7)
                            .checkTournamentComplete(
                                    restState(Lost).bid(p3, Win1).bid(p4, Win2));
                });
    }
    ...
    @Test
    public void mergedLayeredWithEmptyLayer() {
        final TournamentScenario scenario = begin().name("mrgdLrdEmptyLayer")
                .rules(RULES_JP_S1A2G11_NP.withPlayOff(
                        RULES_JP_S1A2G11_NP.getPlayOff()
                                .map(pr -> pr.withConsoleParticipants(
                                        Optional.of(AndWinner2)))))
